...
export const init = (rawOptions?: OptionsType): ReactGrabAPIType => {
	...
	// eslint-disable-next-line @typescript-eslint/no-unused-vars -- omit init-only options that aren't part of SettableOptionsType
	const {
		enabled: _enabled,
	...
	return createRoot((dispose) => {
		...
		const showTemporaryGrabbedBox = (bounds: OverlayBoundsType, element: Element) => {
		...
		const frozenElementsBounds = createMemo((): OverlayBoundsType[] => {
		...
		const pendingShiftSelectionBounds = createMemo((): OverlayBoundsType | undefined => {
			void viewportVersion();
		...
		const selectionBounds = createMemo((): OverlayBoundsType | undefined => {
			void viewportVersion();
		...
		const dragBounds = createMemo((): OverlayBoundsType | undefined => {
			void viewportVersion();
		...
		const dragPreviewBounds = createMemo((): OverlayBoundsType[] => {
			void viewportVersion();
			return dragPreviewElements().map((element) => createElementBounds(element));
		});

		const selectionBoundsMultiple = createMemo((): OverlayBoundsType[] => {
		...
		const recomputeLabelInstance = (instance: SelectionLabelInstance): SelectionLabelInstance => {
			...
			let liveBoundsList: OverlayBoundsType[] | null = null;
		...
		const contextMenuBounds = createMemo((): OverlayBoundsType | null => {
			void viewportVersion();
		...
		const createPerformWithFeedback = (
			...
			return async (action: () => Promise<boolean>): Promise<void> => {
				await withSelectionInteractionLock(async () => {
					...
					let selectionBoundsForLabel: OverlayBoundsType[];
		...
		const api: ReactGrabAPIType = {
			...
			getSource: async (element: Element): Promise<SourceInfoType | null> => {
				const source = await resolveSource(element);
			...
			setOptions: (newOptions: SettableOptionsType) => {
...
export { isInstrumentationActive } from '../octane-adapter.js';
export { DEFAULT_THEME } from './theme.js';

import type * as GrabTypes from '../types.js';

export type Options = GrabTypes.Options;
export type OverlayBounds = GrabTypes.OverlayBounds;
export type ReactGrabRendererProps = GrabTypes.ReactGrabRendererProps;
export type ReactGrabAPI = GrabTypes.ReactGrabAPI;
export type SourceInfo = GrabTypes.SourceInfo;
export type AgentContext = GrabTypes.AgentContext;
export type SettableOptions = GrabTypes.SettableOptions;
export type ContextMenuAction = GrabTypes.ContextMenuAction;
export type ActionContext = GrabTypes.ActionContext;
export type Plugin = GrabTypes.Plugin;
export type PluginConfig = GrabTypes.PluginConfig;
export type PluginHooks = GrabTypes.PluginHooks;
