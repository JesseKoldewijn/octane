				...
				if (a.node.name?.name === 'style') {
					...
					a.get('value').replaceWith(
						t__namespace.jSXExpressionContainer(
							t__namespace.callExpression(t__namespace.arrowFunctionExpression([], value), []),
						),
					);
				}
			});
...
function setAttr$2(path, elem, name, value, { isSVG, dynamic, prevId, isCE, tagName }) {
	...
	if (namespace === 'style') {
		...
		return t__namespace.callExpression(setStyleProperty, [
			...
			t__namespace.isAssignmentExpression(value) && t__namespace.isIdentifier(value.left)
				? value.right
				: value,
		]);
	}
	...
	if (namespace === 'class') {
		return t__namespace.callExpression(
			...
			[
				...
				dynamic
					? value
					: t__namespace.unaryExpression('!', t__namespace.unaryExpression('!', value)),
			],
		);
	}
				...
				for (let i = 0; i < properties.length; i++) {
					...
					if (property.computed) {
						...
						if (r.confident && (typeof r.value === 'string' || typeof r.value === 'number')) {
							property.value = t__namespace.inherits(
								t__namespace.stringLiteral(`${r.value}`),
								property.value,
							);
						}
					...
					if (t__namespace.isObjectProperty(property)) {
						const key = t__namespace.isIdentifier(property.key)
							...
							: property.key.value;
						if (
							t__namespace.isStringLiteral(property.value) ||
							t__namespace.isNumericLiteral(property.value)
						) {
		...
		(a) =>
			...
			!a.node.value.expression.properties.some(
				(p) =>
					...
					p.computed ||
					(t__namespace.isStringLiteral(p.key) &&
							...
							results.exprs.unshift(
								t__namespace.expressionStatement(
									t__namespace.callExpression(
										t__namespace.memberExpression(
											elem,
											t__namespace.identifier('addEventListener'),
										),
...
function wrapDynamics$1(path, dynamics) {
	...
	return t__namespace.expressionStatement(
		t__namespace.callExpression(effectWrapperId, [
			...
			t__namespace.objectExpression(
				properties.map((id) =>
					t__namespace.objectProperty(id, t__namespace.identifier('undefined')),
				),
			),
		]),
	);
...
function escapeExpression(path, expression, attr, escapeLiterals) {
	if (
		...
		t__namespace.isCallExpression(expression) &&
		t__namespace.isFunction(expression.callee)
	) {
...
function normalizeAttributes(path) {
	const attributes = path.get('openingElement').get('attributes'),
		styleAttributes = attributes.filter(
			(a) =>
				t__namespace.isJSXNamespacedName(a.node.name) && a.node.name.namespace.name === 'style',
		),
		classNamespaceAttributes = attributes.filter(
			(a) =>
				t__namespace.isJSXNamespacedName(a.node.name) && a.node.name.namespace.name === 'class',
		);
		...
		for (let i = 0; i < classAttributes.length; i++) {
			...
			if (!t__namespace.isJSXExpressionContainer(attr.value)) {
				...
				if (attr.name.name === 'classList') {
					if (
						t__namespace.isObjectExpression(expr) &&
						!expr.properties.some((p) => t__namespace.isSpreadElement(p))
					) {
...
function transformClasslistObject(path, expr, values, quasis) {
	expr.properties.forEach((prop, i) => {
		...
		if (t__namespace.isBooleanLiteral(prop.value)) {
			...
			values.push(
				t__namespace.conditionalExpression(prop.value, key, t__namespace.stringLiteral('')),
			);
			quasis.push(t__namespace.templateElement({ raw: isLast ? '' : ' ' }));
		}
	});
				...
				t__namespace.callExpression(
					...
					[
						t__namespace.arrayExpression(
							Array.from(path.scope.data.events).map((e) => t__namespace.stringLiteral(e)),
						),
					],
				),
			),
