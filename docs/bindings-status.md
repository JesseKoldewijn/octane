...
The central status table for the 27 `@octanejs/*` framework bindings.
...
| [`@octanejs/dexie`](#octanejsdexie) | `dexie-react-hooks@4.4.0` | Port of the public dexie-react-hooks surface: useObservable, useLiveQuery, useSuspendingObservable, useSuspendingLiveQuery, usePermissions, and useDocument, with Dexie's framework-neutral API re-exported from the package root. | Suspending hooks integrate with Octane's use() rather than React's use() or thrown-promise implementation details; Hook call-site slots are forwarded through Octane's compiler binding ABI; useDocument requires consumers to install and import y-dexie and yjs before using the hook; those integrations remain optional | Dexie's core exports are available during SSR; live-query subscriptions are client-side and server rendering returns the configured default for non-suspending hooks. Dedicated SSR and hydration tests are not yet included. | 2026-07-16 |
...
| [`@octanejs/three`](#octanejsthree) | `@react-three/fiber@9.6.1 (2a528745)` | Technical-preview Milestones 0, 2, 3, and 4 surface: renderer configuration and DOM Canvas boundary, compiler ABI and renderer-local Three intrinsic types, catalogue and both extend forms, primitive/args construction, Three prop application, attachment, ordered placement/recreation, retained visibility, lifecycle/ref delivery, ownership-aware disposal, promise-returning programmatic roots, callable root state, scene/camera/raycaster and resize/DPR/viewport configuration, shadows/colors, one shared frame loop, global effects, useStore/useThree/useFrame/useGraph and managed-instance helpers, the ray/pointer event system with DOM sources and custom managers, and a deterministic testing harness. Asset and portal surfaces remain deferred. | Octane owns component execution, hooks, context, scheduling, Suspense, refs, and effects instead of embedding React Reconciler; The programmatic root renders an Octane component plus props rather than a React element descriptor; The upstream callable store selector remains order-based because dynamic function calls cannot receive compiler slots; compiler-visible useStore(selector) and useThree(selector) preserve Octane's conditional-hook semantics; buildGraph omits unnamed mesh and material entries, plus array-valued material entries, instead of publishing empty or undefined keys; Removing a pierced prop resets its original nested target; R3F 9.6.1 mistakenly writes that default to the leaf key on the root object; Reconstructing a captured or hovered object rewrites nested stored intersections to the replacement; R3F 9.6.1 updates only the outer hover identity and capture-map key, which leaves captured delivery pointing at the retired object; Hidden retained Activity subtrees are excluded from recursive raycasts; Three r172 ignores Object3D.visible during raycasting, so R3F 9.6.1 can otherwise pierce a hidden descendant through an interactive visible ancestor | Three scene modules are classified client-only and Canvas.children is configured as an omitted server region. Package-level DOM-shell server output and hydration adoption remain Milestone 7 work. | 2026-07-16 |
...
[`packages/apollo-client`](../packages/apollo-client) `0.1.2` — ports `@apollo/client@4.2.6`. Status data: [`packages/apollo-client/status.json`](../packages/apollo-client/status.json).
...
[`packages/base-ui`](../packages/base-ui) `0.1.5` — ports `@base-ui/react@1.6.0`. Status data: [`packages/base-ui/status.json`](../packages/base-ui/status.json).
...
[`packages/dexie`](../packages/dexie) `0.1.0` — ports `dexie-react-hooks@4.4.0`. Status data: [`packages/dexie/status.json`](../packages/dexie/status.json).
...
SSR / hydration: Dexie's core exports are available during SSR; live-query subscriptions are client-side and server rendering returns the configured default for non-suspending hooks. Dedicated SSR and hydration tests are not yet included.

Scope/evidence last checked: 2026-07-16.

## @octanejs/dnd-kit

[`packages/dnd-kit`](../packages/dnd-kit) `0.1.2` — ports `@dnd-kit/react@0.5.0`. Status data: [`packages/dnd-kit/status.json`](../packages/dnd-kit/status.json).
...
[`packages/floating-ui`](../packages/floating-ui) `0.1.6` — ports `@floating-ui/react@0.27.19`. Status data: [`packages/floating-ui/status.json`](../packages/floating-ui/status.json).
...
[`packages/hook-form`](../packages/hook-form) `0.1.4` — ports `react-hook-form@7.81.0`. Status data: [`packages/hook-form/status.json`](../packages/hook-form/status.json).
...
[`packages/i18next`](../packages/i18next) `0.1.2` — ports `react-i18next@17.0.9`. Status data: [`packages/i18next/status.json`](../packages/i18next/status.json).
...
[`packages/jotai`](../packages/jotai) `0.1.4` — ports `jotai@2.20.1`. Status data: [`packages/jotai/status.json`](../packages/jotai/status.json).
...
[`packages/lexical`](../packages/lexical) `0.1.6` — ports `@lexical/react@0.46.0`. Status data: [`packages/lexical/status.json`](../packages/lexical/status.json).
...
[`packages/lucide`](../packages/lucide) `0.1.2` — ports `lucide-react@1.24.0`. Status data: [`packages/lucide/status.json`](../packages/lucide/status.json).
...
[`packages/mdx`](../packages/mdx) `0.1.4` — ports `@mdx-js/mdx@3.1.1`. Status data: [`packages/mdx/status.json`](../packages/mdx/status.json).
...
[`packages/motion`](../packages/motion) `0.1.6` — ports `motion@12.40.0`. Status data: [`packages/motion/status.json`](../packages/motion/status.json).
...
[`packages/radix`](../packages/radix) `0.1.6` — ports `radix-ui@1.6.1`. Status data: [`packages/radix/status.json`](../packages/radix/status.json).
...
[`packages/recharts`](../packages/recharts) `0.1.4` — ports `recharts@3.9.2`. Status data: [`packages/recharts/status.json`](../packages/recharts/status.json).
...
[`packages/redux`](../packages/redux) `0.1.4` — ports `react-redux@9.3.0`. Status data: [`packages/redux/status.json`](../packages/redux/status.json).
...
[`packages/redux-toolkit`](../packages/redux-toolkit) `0.1.2` — ports `@reduxjs/toolkit@2.12.0`. Status data: [`packages/redux-toolkit/status.json`](../packages/redux-toolkit/status.json).
...
[`packages/remix-router`](../packages/remix-router) `0.1.3` — ports `react-router@8.2.0`. Status data: [`packages/remix-router/status.json`](../packages/remix-router/status.json).
...
[`packages/sonner`](../packages/sonner) `0.1.2` — ports `sonner@2.0.7`. Status data: [`packages/sonner/status.json`](../packages/sonner/status.json).
...
[`packages/stylex`](../packages/stylex) `0.1.6` — ports `@stylexjs/stylex@0.19.0`. Status data: [`packages/stylex/status.json`](../packages/stylex/status.json).
...
[`packages/tanstack-query`](../packages/tanstack-query) `0.1.6` — ports `@tanstack/react-query@5.101.0`. Status data: [`packages/tanstack-query/status.json`](../packages/tanstack-query/status.json).
...
[`packages/tanstack-router`](../packages/tanstack-router) `0.1.6` — ports `@tanstack/react-router@1.170.16`. Status data: [`packages/tanstack-router/status.json`](../packages/tanstack-router/status.json).
...
[`packages/tanstack-table`](../packages/tanstack-table) `0.1.4` — ports `@tanstack/react-table@8.21.3`. Status data: [`packages/tanstack-table/status.json`](../packages/tanstack-table/status.json).
...
[`packages/tanstack-virtual`](../packages/tanstack-virtual) `0.1.4` — ports `@tanstack/react-virtual@3.14.5`. Status data: [`packages/tanstack-virtual/status.json`](../packages/tanstack-virtual/status.json).
...
[`packages/testing-library`](../packages/testing-library) `0.1.4` — ports `@testing-library/react` (unpinned). Status data: [`packages/testing-library/status.json`](../packages/testing-library/status.json).
...
Technical-preview Milestones 0, 2, 3, and 4 surface: renderer configuration and DOM Canvas boundary, compiler ABI and renderer-local Three intrinsic types, catalogue and both extend forms, primitive/args construction, Three prop application, attachment, ordered placement/recreation, retained visibility, lifecycle/ref delivery, ownership-aware disposal, promise-returning programmatic roots, callable root state, scene/camera/raycaster and resize/DPR/viewport configuration, shadows/colors, one shared frame loop, global effects, useStore/useThree/useFrame/useGraph and managed-instance helpers, the ray/pointer event system with DOM sources and custom managers, and a deterministic testing harness. Asset and portal surfaces remain deferred.
...
SSR / hydration: Three scene modules are classified client-only and Canvas.children is configured as an omitted server region. Package-level DOM-shell server output and hydration adoption remain Milestone 7 work.
...
- The checked-in crosswalk classifies 90 upstream public exports and 157 executable upstream tests with zero unclassified cases; Milestone 4 adds public raycast, bubbling, propagation, hover, misses, capture, event-source, custom-manager, reconstruction, testing-helper, differential, and browser evidence.
- Asset loading, portals and portal event layers, full Canvas SSR/hydration, and XR/HMR hardening follow in later milestones.
...
[`packages/visx`](../packages/visx) `0.1.1` — ports `@visx/visx@4.0.0 + master@485c035`. Status data: [`packages/visx/status.json`](../packages/visx/status.json).
...
[`packages/zustand`](../packages/zustand) `0.1.6` — ports `zustand@5.0.14`. Status data: [`packages/zustand/status.json`](../packages/zustand/status.json).
