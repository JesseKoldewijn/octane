// Slot mechanics for @octanejs/monaco-editor-octane's plain-`.ts` hooks. The
// Octane compiler injects a per-call-site Symbol as the final argument of every
// custom-hook call from compiled `.tsrx`. Binding hooks split that argument and
// derive one stable sub-slot for each base hook they compose.
...
export function subSlot(slot: symbol | undefined, tag: string): symbol {
	if (slot === undefined) {
		...
		if (bare === undefined) {
			bare = Symbol.for(`@octanejs/monaco-editor-octane:${tag}`);
	...
	if (result === undefined) {
		result = Symbol.for(`${slot.description ?? ''}:@octanejs/monaco-editor-octane:${tag}`);
