...
| [`@octanejs/monaco-editor-octane`](#octanejsmonaco-editor-octane) | `@monaco-editor/react@4.8.0-rc.3 (f7ef2e6)` | Complete against the pinned @monaco-editor/react 4.8.0-rc.3 public surface: default Editor, DiffEditor, loader re-export, useMonaco, Monaco/Theme types, and Editor/DiffEditor prop/callback types. @monaco-editor/loader is reused unchanged. | loading accepts OctaneNode \| string instead of React.ReactNode; Internal MonacoContainer uses Octane ref on the host div; upstream's private _ref prop is not carried forward; 'use client' directives are omitted; editors remain browser-only via effects; CSS style objects use plain records rather than React.CSSProperties; DiffEditor dispose clears editorRef / preventCreation / isEditorReady like Editor; upstream DiffEditor at the pin still omits that remount-safe reset | Supported and tested: Editor/DiffEditor server-render the loading shell (section + Loading) and do not call loader.init or monaco.editor.create on the server. hydrateRoot adopts that shell; client effects run init afterward. | 2026-08-09 |
...
| [`@octanejs/rxjs`](#octanejsrxjs) | `@react-rxjs/core + @react-rxjs/utils@0.10.8 / 0.9.7` | Core bind/state/Subscribe APIs and the complete @react-rxjs/utils surface. | StateObservable values are not JSX nodes; render them through useStateObservable or bind; @react-rxjs/dom is omitted because octane batches updates without ReactDOM.unstable_batchedUpdates | useSyncExternalStore supplies the state snapshot during server rendering; browser-only ReactDOM batching helpers are intentionally absent. | 2026-07-30 |
...
[`packages/animejs`](../packages/animejs) `0.0.2` — ports `animejs@4.5.0`. Status data: [`packages/animejs/status.json`](../packages/animejs/status.json).
...
[`packages/apollo-client`](../packages/apollo-client) `0.1.27` — ports `@apollo/client@4.2.6`. Status data: [`packages/apollo-client/status.json`](../packages/apollo-client/status.json).
...
[`packages/aria`](../packages/aria) `0.0.26` — ports `react-aria@3.50.0`. Status data: [`packages/aria/status.json`](../packages/aria/status.json).
...
[`packages/base-ui`](../packages/base-ui) `0.1.30` — ports `@base-ui/react@1.6.0`. Status data: [`packages/base-ui/status.json`](../packages/base-ui/status.json).

Alpha, in progress: 34 of 43 upstream subpaths. The foundation, overlay infrastructure, hover/focus interaction layer, list-navigation/typeahead layer and popup viewport have landed, along with Dialog, AlertDialog, Popover, Tooltip, PreviewCard, the full form-control set, and the standalone Button/DirectionProvider/CSPProvider/useMediaQuery utilities — ported at full fidelity and differential-verified against the real `@base-ui/react`. Menu is COMPLETE at all 20 upstream parts — Root/Trigger/Portal/Positioner/Popup, the full item family (Item, LinkItem, CheckboxItem, RadioGroup/RadioItem and their indicators, Group/GroupLabel, Separator), Arrow, Backdrop, Viewport, and SubmenuRoot/SubmenuTrigger. Menubar and ContextMenu have landed too, so the whole menu family is complete. Toast is complete at all 11 parts, including the imperative toast manager, auto-dismiss timers with hover/focus pausing, and promise toasts; swipe-to-dismiss is covered too. Collapsible is complete at all 3 upstream parts (Root/Trigger/Panel), including upstream's <Activity> animation-resume suppression — octane's Activity has React's contract (hidden subtrees keep state and DOM while effects tear down and re-fire on reveal), so that path is ported rather than skipped and is the first component carrying a port of Base UI's OWN test file, under tests/upstream/ — 16 of its 35 upstream cases, being every case that does not require real layout measurement, CSS transition/keyframe sequencing, React.Activity, or find-in-page `beforematch`. Accordion is complete at all 5 upstream parts (Root/Item/Header/Trigger/Panel), built directly on Collapsible — each Item runs its own useCollapsibleRoot and provides CollapsibleRootContext, exactly as upstream does. It carries 18 tests: 15 ported from upstream's own suite plus 3 written to close a real gap — upstream only exercises single-vs-multiple toggling inside jsdom-skipped blocks, so removing the single-mode collapse branch passed every ported case.
...
[`packages/cmdk`](../packages/cmdk) `0.1.15` — ports `cmdk@1.1.1`. Status data: [`packages/cmdk/status.json`](../packages/cmdk/status.json).
...
[`packages/devtools`](../packages/devtools) `0.0.20` — ports `octane@workspace`. Status data: [`packages/devtools/status.json`](../packages/devtools/status.json).
...
[`packages/dexie`](../packages/dexie) `0.1.25` — ports `dexie-react-hooks@4.4.0`. Status data: [`packages/dexie/status.json`](../packages/dexie/status.json).
...
[`packages/dnd-kit`](../packages/dnd-kit) `0.1.27` — ports `@dnd-kit/react@0.5.0`. Status data: [`packages/dnd-kit/status.json`](../packages/dnd-kit/status.json).
...
[`packages/electron`](../packages/electron) `0.0.10` — ports `electron@43.2.0`. Status data: [`packages/electron/status.json`](../packages/electron/status.json).
...
[`packages/floating-ui`](../packages/floating-ui) `0.1.31` — ports `@floating-ui/react@0.27.19`. Status data: [`packages/floating-ui/status.json`](../packages/floating-ui/status.json).
...
[`packages/hook-form`](../packages/hook-form) `0.1.29` — ports `react-hook-form@7.81.0`. Status data: [`packages/hook-form/status.json`](../packages/hook-form/status.json).
...
[`packages/i18next`](../packages/i18next) `0.1.27` — ports `react-i18next@17.0.9`. Status data: [`packages/i18next/status.json`](../packages/i18next/status.json).
...
[`packages/inertia`](../packages/inertia) `0.0.1` — ports `@inertiajs/react@3.6.1`. Status data: [`packages/inertia/status.json`](../packages/inertia/status.json).
...
[`packages/jotai`](../packages/jotai) `0.1.29` — ports `jotai@2.20.2`. Status data: [`packages/jotai/status.json`](../packages/jotai/status.json).
...
[`packages/lexical`](../packages/lexical) `0.1.31` — ports `@lexical/react@0.46.0`. Status data: [`packages/lexical/status.json`](../packages/lexical/status.json).
...
[`packages/livestore`](../packages/livestore) `0.0.2` — ports `@livestore/react@0.4.0`. Status data: [`packages/livestore/status.json`](../packages/livestore/status.json).
...
[`packages/lucide`](../packages/lucide) `0.1.27` — ports `lucide-react@1.24.0`. Status data: [`packages/lucide/status.json`](../packages/lucide/status.json).
...
[`packages/mantine-hooks`](../packages/mantine-hooks) `0.1.13` — ports `@mantine/hooks@9.5.0`. Status data: [`packages/mantine-hooks/status.json`](../packages/mantine-hooks/status.json).
...
[`packages/mdx`](../packages/mdx) `0.1.29` — ports `@mdx-js/mdx@3.1.1`. Status data: [`packages/mdx/status.json`](../packages/mdx/status.json).
...
[`packages/mobx`](../packages/mobx) `0.1.13` — ports `mobx-react-lite@4.1.1`. Status data: [`packages/mobx/status.json`](../packages/mobx/status.json).
...
[`packages/monaco-editor-octane`](../packages/monaco-editor-octane) `0.0.1` — ports `@monaco-editor/react@4.8.0-rc.3 (f7ef2e6)`. Status data: [`packages/monaco-editor-octane/status.json`](../packages/monaco-editor-octane/status.json).

Complete against the pinned @monaco-editor/react 4.8.0-rc.3 public surface: default Editor, DiffEditor, loader re-export, useMonaco, Monaco/Theme types, and Editor/DiffEditor prop/callback types. @monaco-editor/loader is reused unchanged.

Known divergences:

- loading accepts OctaneNode | string instead of React.ReactNode.
- Internal MonacoContainer uses Octane ref on the host div; upstream's private _ref prop is not carried forward.
- 'use client' directives are omitted; editors remain browser-only via effects.
- CSS style objects use plain records rather than React.CSSProperties.
- DiffEditor dispose clears editorRef / preventCreation / isEditorReady like Editor; upstream DiffEditor at the pin still omits that remount-safe reset.

SSR / hydration: Supported and tested: Editor/DiffEditor server-render the loading shell (section + Loading) and do not call loader.init or monaco.editor.create on the server. hydrateRoot adopts that shell; client effects run init afterward.

Scope/evidence last checked: 2026-08-09.

- monaco-editor peer range matches upstream (>= 0.25.0 < 1). Oracle pin is monaco-editor@0.56.0.
- Upstream Vitest suite is snapshot-only (4 files under tests/upstream). Port-authored lifecycle/useMonaco live under tests/runtime (ordinary shards). Differential, SSR/hydration, Chromium browser, and example e2e supply additional behavioral parity.
- Library onChange prop name is preserved (model content callback), not rewritten to onInput.
- Worker configuration stays in the consuming app. v1 default (examples/monaco-playground + package Chromium harness) is npm monaco-editor@0.56.0 via Vite MonacoEnvironment + ?worker and loader.config({ monaco }); CDN AMD paths remain a documented alternate in README.

See also: [`packages/monaco-editor-octane/UPSTREAM.md`](../packages/monaco-editor-octane/UPSTREAM.md)

## @octanejs/motion

[`packages/motion`](../packages/motion) `0.1.31` — ports `motion@12.42.2`. Status data: [`packages/motion/status.json`](../packages/motion/status.json).
...
[`packages/nuqs`](../packages/nuqs) `0.1.21` — ports `nuqs@2.9.1`. Status data: [`packages/nuqs/status.json`](../packages/nuqs/status.json).
...
[`packages/phosphor-icons`](../packages/phosphor-icons) `0.0.12` — ports `@phosphor-icons/react@2.1.10`. Status data: [`packages/phosphor-icons/status.json`](../packages/phosphor-icons/status.json).
...
[`packages/radix`](../packages/radix) `0.1.31` — ports `radix-ui@1.6.4`. Status data: [`packages/radix/status.json`](../packages/radix/status.json).
...
[`packages/rainbowkit`](../packages/rainbowkit) `0.0.12` — ports `@rainbow-me/rainbowkit@2.2.11`. Status data: [`packages/rainbowkit/status.json`](../packages/rainbowkit/status.json).
...
[`packages/react-error-boundary`](../packages/react-error-boundary) `0.1.13` — ports `react-error-boundary@6.1.2`. Status data: [`packages/react-error-boundary/status.json`](../packages/react-error-boundary/status.json).
...
[`packages/react-map-gl`](../packages/react-map-gl) `0.0.4` — ports `@vis.gl/react-mapbox@8.1.2 (b1e46fcf)`. Status data: [`packages/react-map-gl/status.json`](../packages/react-map-gl/status.json).
...
[`packages/recharts`](../packages/recharts) `0.1.29` — ports `recharts@3.9.2`. Status data: [`packages/recharts/status.json`](../packages/recharts/status.json).
...
[`packages/redux`](../packages/redux) `0.1.29` — ports `react-redux@9.3.0`. Status data: [`packages/redux/status.json`](../packages/redux/status.json).
...
[`packages/redux-toolkit`](../packages/redux-toolkit) `0.1.27` — ports `@reduxjs/toolkit@2.12.0`. Status data: [`packages/redux-toolkit/status.json`](../packages/redux-toolkit/status.json).
...
[`packages/remix-router`](../packages/remix-router) `0.1.28` — ports `react-router@8.2.0`. Status data: [`packages/remix-router/status.json`](../packages/remix-router/status.json).
...
[`packages/rxjs`](../packages/rxjs) `0.1.11` — ports `@react-rxjs/core + @react-rxjs/utils@0.10.8 / 0.9.7`. Status data: [`packages/rxjs/status.json`](../packages/rxjs/status.json).
...
[`packages/shadcn`](../packages/shadcn) `0.0.17` — ports `shadcn-ui/ui (radix base)@4baadbc6517070ae8f8feb2c97037adc2b305544 + shadcn@4.14.1`. Status data: [`packages/shadcn/status.json`](../packages/shadcn/status.json).
...
[`packages/solana-react`](../packages/solana-react) `0.0.1` — ports `@solana/react@7.0.0`. Status data: [`packages/solana-react/status.json`](../packages/solana-react/status.json).
...
[`packages/sonner`](../packages/sonner) `0.1.27` — ports `sonner@2.0.7`. Status data: [`packages/sonner/status.json`](../packages/sonner/status.json).
...
[`packages/streamdown`](../packages/streamdown) `0.1.11` — ports `streamdown@2.5.0`. Status data: [`packages/streamdown/status.json`](../packages/streamdown/status.json).
...
[`packages/styled-components`](../packages/styled-components) `0.1.24` — ports `styled-components@6.4.3`. Status data: [`packages/styled-components/status.json`](../packages/styled-components/status.json).
...
[`packages/stylex`](../packages/stylex) `0.1.31` — ports `@stylexjs/stylex@0.19.0`. Status data: [`packages/stylex/status.json`](../packages/stylex/status.json).
...
[`packages/tanstack-ai`](../packages/tanstack-ai) `0.0.26` — ports `@tanstack/ai-react@0.17.0`. Status data: [`packages/tanstack-ai/status.json`](../packages/tanstack-ai/status.json).
...
[`packages/tanstack-devtools`](../packages/tanstack-devtools) `0.0.26` — ports `@tanstack/react-devtools@0.10.7`. Status data: [`packages/tanstack-devtools/status.json`](../packages/tanstack-devtools/status.json).
...
[`packages/tanstack-form`](../packages/tanstack-form) `0.0.26` — ports `@tanstack/react-form@1.33.2`. Status data: [`packages/tanstack-form/status.json`](../packages/tanstack-form/status.json).
...
[`packages/tanstack-hotkeys`](../packages/tanstack-hotkeys) `0.0.21` — ports `@tanstack/react-hotkeys@0.10.0`. Status data: [`packages/tanstack-hotkeys/status.json`](../packages/tanstack-hotkeys/status.json).
...
[`packages/tanstack-pacer`](../packages/tanstack-pacer) `0.0.21` — ports `@tanstack/react-pacer@0.22.1`. Status data: [`packages/tanstack-pacer/status.json`](../packages/tanstack-pacer/status.json).
...
[`packages/tanstack-query`](../packages/tanstack-query) `0.1.31` — ports `@tanstack/react-query@5.101.3`. Status data: [`packages/tanstack-query/status.json`](../packages/tanstack-query/status.json).
...
[`packages/tanstack-router`](../packages/tanstack-router) `0.1.31` — ports `@tanstack/react-router@1.170.18`. Status data: [`packages/tanstack-router/status.json`](../packages/tanstack-router/status.json).
...
[`packages/tanstack-router-ssr-query`](../packages/tanstack-router-ssr-query) `0.0.21` — ports `@tanstack/react-router-ssr-query@1.167.1`. Status data: [`packages/tanstack-router-ssr-query/status.json`](../packages/tanstack-router-ssr-query/status.json).
...
[`packages/tanstack-store`](../packages/tanstack-store) `0.0.26` — ports `@tanstack/react-store@0.11.0`. Status data: [`packages/tanstack-store/status.json`](../packages/tanstack-store/status.json).
...
[`packages/tanstack-table`](../packages/tanstack-table) `0.1.29` — ports `@tanstack/react-table@9.0.0-beta.58`. Status data: [`packages/tanstack-table/status.json`](../packages/tanstack-table/status.json).
...
[`packages/tanstack-virtual`](../packages/tanstack-virtual) `0.1.29` — ports `@tanstack/react-virtual@3.14.5`. Status data: [`packages/tanstack-virtual/status.json`](../packages/tanstack-virtual/status.json).
...
[`packages/tauri`](../packages/tauri) `0.0.15` — ports `@tauri-apps/api@2.11.1`. Status data: [`packages/tauri/status.json`](../packages/tauri/status.json).
...
[`packages/testing-library`](../packages/testing-library) `0.1.29` — ports `@testing-library/react` (unpinned). Status data: [`packages/testing-library/status.json`](../packages/testing-library/status.json).
...
[`packages/three`](../packages/three) `0.1.25` — ports `@react-three/fiber@9.6.1 (2a528745)`. Status data: [`packages/three/status.json`](../packages/three/status.json).
...
[`packages/tiptap`](../packages/tiptap) `0.0.26` — ports `@tiptap/react@3.28.0`. Status data: [`packages/tiptap/status.json`](../packages/tiptap/status.json).
...
[`packages/usehooks-ts`](../packages/usehooks-ts) `0.0.13` — ports `usehooks-ts@3.1.1`. Status data: [`packages/usehooks-ts/status.json`](../packages/usehooks-ts/status.json).
...
[`packages/valtio`](../packages/valtio) `0.1.15` — ports `valtio@2.3.2`. Status data: [`packages/valtio/status.json`](../packages/valtio/status.json).
...
[`packages/visx`](../packages/visx) `0.1.26` — ports `@visx/visx@4.0.0 + master@485c035`. Status data: [`packages/visx/status.json`](../packages/visx/status.json).
...
[`packages/wagmi`](../packages/wagmi) `0.0.12` — ports `wagmi@3.7.4`. Status data: [`packages/wagmi/status.json`](../packages/wagmi/status.json).
...
[`packages/zustand`](../packages/zustand) `0.1.31` — ports `zustand@5.0.14`. Status data: [`packages/zustand/status.json`](../packages/zustand/status.json).
